                                          ...
                                          size_t &nargt, bool &isVa, Type *&lrt, bool &retboxed, bool &static_rt)
{
    ...
    if (tvars == jl_emptysvec) {
        ...
        if (!static_rt && sparam_vals != NULL) {
            assert((jl_is_svec(tvars) ? jl_svec_len(tvars) : 1) == jl_svec_len(sparam_vals));
        ...
        jl_codectx_t *ctx)
{
    ...
    for (size_t ai = 0; ai < nargs; ai++) {
        ...
        if (ctx->spvals_ptr == NULL && !toboxed && tvars != jl_emptysvec && jl_has_typevars(jargty)) {
            assert((jl_is_svec(tvars) ? jl_svec_len(tvars) : 1) == jl_svec_len(ctx->linfo->sparam_vals));
